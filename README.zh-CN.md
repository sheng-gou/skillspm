...
`skills install` 会读取 `skills.yaml` 并安装这套环境。  
`skills freeze` 会把解析后的状态写入 `skills.lock`。  
`skills sync` 会把已安装的环境同步到一个或多个 agent。
...
- 本地 skill 用 `path`
- 通过索引解析的 skill 用 `id + version + source`
