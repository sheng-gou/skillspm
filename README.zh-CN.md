...
* [`examples/source-aware-live`](examples/source-aware-live/README.md)：同一个 workspace 里同时包含一个 source-backed skill 和一个本地 path skill
* [`examples/pack-transfer`](examples/pack-transfer/README.md)：先从声明式 source 安装，再写出目录 pack，最后通过顶层 `packs[]` 恢复
...
* `sources`：可选的声明式 live source，目前支持 `index` 和受限的公开 HTTPS `git`
...
Phase 1 只支持匿名公开 `https://` git source，并且这类 source 只支持一种固定布局。只接受纯仓库 URL：`file://`、`ssh://`、`git@host:repo`、带内嵌凭据的 URL、非空 query string，以及非空 `#` fragment 都会被拒绝。安装时，`skillspm` 会隔离 git 配置、禁用凭据提示与 helper，并阻止 `file://` transport，因此像 `url.*.insteadOf` 这样的环境重写规则不能绕过这条策略。
